...
// ---------------------------------------------------------------------------
// Entity-array merge (hosts, keys, groups)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// String-array merge (customGroups)
// ---------------------------------------------------------------------------
  ...
  for (const key of allKeys) {
    ...
    if (!lChanged && !rChanged) {
      ...
      if (
        ...
        (isIdArray(lVal) || isIdArray(rVal))
      ) {
        // Array of objects with `id` — entity merge
...
export function mergeSyncPayloads(
  ...
  const groups = mergeEntityArrays(b.groups ?? [], local.groups ?? [], remote.groups ?? []);
