...
export class ProviderConfigComponent implements OnInit, OnDestroy, OnChanges {
    ...
    ngOnChanges(changes: SimpleChanges): void {
        if (changes['focusProvider'] || changes['singleProviderMode']) {
            this.applyFocusedProvider()
        }
    }
     ...
     * 创建默认配置
     */
    private createDefaultConfig(fields: ProviderField[]): Record<string, unknown> {
     ...
     * 获取字段类型
     */
    getFieldType(field: ProviderField): string {
     ...
     * 获取选项
     */
    getFieldOptions(field: ProviderField): string[] {
     ...
     * 检查是否是密码字段
     */
    isPasswordField(field: ProviderField): boolean {
     ...
     * 检查是否必填
     */
    isRequired(field: ProviderField): boolean {
     ...
     * 获取提供商模板（支持云端和本地）
     */
    getProviderTemplate(providerName: string): ProviderTemplate | undefined {
    ...
    getVisibleCloudProviders(): string[] {
        return this.filterProviderNames(Object.keys(this.cloudProviderTemplates))
    }

    getVisibleLocalProviders(): string[] {
        return this.filterProviderNames(Object.keys(this.localProviderTemplates))
    }
     ...
     * 更新配置值
     */
    updateConfigValue(providerName: string, key: string, value: unknown): void {
        if (!this.configs[providerName]) {
    ...
    private applyFocusedProvider(): void {
        const preferredProvider = this.resolvePreferredProvider()
        ...
        if (this.singleProviderMode || !this.expandedProvider) {
            this.expandedProvider = preferredProvider
        }
    }

    private resolvePreferredProvider(): string {
        if (this.focusProvider && this.isKnownProvider(this.focusProvider)) {
            return this.focusProvider
        }

        if (this.selectedProvider && this.isKnownProvider(this.selectedProvider)) {
            return this.selectedProvider
        }

        const configuredProvider = Object.keys(this.configs).find(name => this.isKnownProvider(name))
        if (configuredProvider) {
            return configuredProvider
        }

        return Object.keys(this.cloudProviderTemplates)[0] || Object.keys(this.localProviderTemplates)[0] || ''
    }

    private filterProviderNames(names: string[]): string[] {
        if (!this.singleProviderMode) {
            return names
        }

        const preferredProvider = this.resolvePreferredProvider()
        return preferredProvider ? names.filter(name => name === preferredProvider) : []
    }

    private isKnownProvider(providerName: string): boolean {
        return providerName in this.cloudProviderTemplates || providerName in this.localProviderTemplates
    }
}
