 ...
 * 格式化字节数组
 */
export function formatBytes(bytes: number[], separator = ' '): string {
 ...
 * 格式化JSON（美化输出）
 */
export function formatJson(json: string | object, indent = 2): string {
    try {
 ...
 * 格式化列表为文本
 */
export function formatList(items: string[], delimiter = ', '): string {
 ...
 * 格式化代码块
 */
export function formatCodeBlock(code: string, language = 'text'): string {
    return '```' + language + '\n' + code + '\n```'
}
