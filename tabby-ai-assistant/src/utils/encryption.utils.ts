...
export function encrypt(text: string, key: string): string {
    if (text === null || text === undefined || key === undefined) {
...
export function decrypt(encryptedText: string, key: string): string {
    if (encryptedText === null || encryptedText === undefined || key === undefined) {
 ...
 * 使用PBKDF2派生密钥
 */
export function deriveKey(password: string, salt: string, iterations = 10000): string {
    if (!password || !salt) {
 ...
 * 清除敏感数据
 */
export function clearSensitiveData(obj: Record<string, unknown> | null | undefined): void {
    if (!obj) {return}
 ...
 * 生成加密的配置对象
 */
export function createSecureConfig(data: unknown, masterPassword: string): { encrypted: string; salt: string } {
 ...
 * 解密配置对象
 */
export function parseSecureConfig(encryptedData: string, salt: string, masterPassword: string): unknown {
 ...
 * 验证密码
 */
export function verifyPassword(password: string, hashValue: string, salt: string): boolean {
    if (!password || !hashValue || !salt) {
        return false
    }

    return verifyHash(password, hashValue, salt)
}
