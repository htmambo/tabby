...
export class TerminalManagerService {
    ...
    promptDetection$ = this.promptDetectionSubject.asObservable()
     ...
     * 向当前终端发送命令
     */
    sendCommand(command: string, execute = true): boolean {
     ...
     * 向指定终端发送命令
     */
    sendCommandToTerminal(terminal: TerminalTab, command: string, execute = true): boolean {
        try {
     ...
     * 向指定索引的终端发送命令
     */
    sendCommandToIndex(index: number, command: string, execute = true): boolean {
     ...
     * 从指定终端读取输出
     */
    private readTerminalOutputFromTerminal(terminal: TerminalTab, lines = 50): string {
        if (!terminal?.frontend) {
            return ''
        }
     ...
     * 开始持续监控终端
     */
    startContinuousMonitoring(intervalMs = 5000): void {
        const terminals = this.getAllTerminals()
     ...
     * 读取终端输出（快捷键功能需要）
     */
    readTerminalOutput(lines = 50, terminalIndex?: number): string {
        try {
            const terminals = this.getAllTerminals()
            ...
            const buffer = core.buffer || (core.terminal?.buffer)
