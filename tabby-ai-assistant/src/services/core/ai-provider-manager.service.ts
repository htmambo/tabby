...
export class AiProviderManagerService implements ProviderManager {
     ...
     * 检查所有提供商健康状态（使用缓存）
     */
    async checkAllProvidersHealth(forceRefresh = false): Promise<{ provider: string; status: HealthStatus; latency?: number; cached?: boolean }[]> {
        const results: { provider: string; status: HealthStatus; latency?: number; cached?: boolean }[] = []

        // 并行执行所有健康检查
        const checks = Array.from(this.providers.entries()).map(async ([name, _provider]) => {
     ...
     * 获取单个提供商的健康状态（使用缓存）
     */
    async getProviderHealthStatus(providerName: string, forceRefresh = false): Promise<HealthStatus> {
