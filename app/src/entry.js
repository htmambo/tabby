...
import { BOOTSTRAP_DATA } from '../../tabby-core/src/api/mainProcess';
...
async function bootstrap(bootstrapData, plugins, safeMode = false) {
    if (safeMode) {
        plugins = plugins.filter(x => x.isBuiltin);
    }
    const pluginModules = await loadPlugins(plugins, (current, total) => {
        document.querySelector('.progress .bar').style.width = `${100 * current / total}%`; // eslint-disable-line
    });
...
ipc.once('start', async (bootstrapData) => {
    ...
    console.debug('Starting with plugins:', plugins);
    try {
