...
let managedUserPluginsNodeModulesPath = null;
let pluginLookupPaths = [];
const pluginRuntimeRoots = new Set();
...
function pathExistsSync(targetPath) {
    return bridgeIPC.sendSync('bridge:fs:exists-sync', targetPath);
}
function resolveDevelopmentWorkspaceRoot() {
    ...
    for (const candidate of candidates) {
        if (pathExistsSync(path.join(candidate, 'app', 'package.json')) &&
...
const appPath = bridgeIPC.sendSync('bridge:app:get-app-path');
...
const builtinPluginRoots = new Set([
...
nodeModule.prototype.require = (function (query) {
    if (cachedBuiltinModules[query] && isPluginModuleContext(this)) {
        return cachedBuiltinModules[query];
    }
    return originalModuleRequire.call(this, query);
});
function normalizePluginManifest(manifest, pluginName) {
    if (!manifest) {
        return undefined;
    }
    if (!manifest.name) {
...
function isWithinRuntimeRoot(candidatePath) {
...
function registerPluginRuntimeRoot(pluginRoot) {
    if (!pluginRoot) {
        return;
    }
    pluginRuntimeRoots.add(normalizePathForCompare(pluginRoot));
}
function isPluginModuleContext(moduleContext) {
    const visited = new Set();
    let current = moduleContext;
    while (current && typeof current === 'object' && !visited.has(current)) {
        visited.add(current);
        if ((current.filename && isWithinRuntimeRoot(current.filename)) ||
...
function getInitialModuleLookupPaths() {
    const rendererModulePaths = globalThis.module?.paths ?? [];
...
function isBuiltinPluginDir(pluginDir) {
...
function resolveBuiltinPackagePath(packageName) {
...
export function initModuleLookup(userPluginsPath) {
...
async function getCandidateLocationsInPluginDir(pluginDir) {
...
async function getPluginCandidateLocation(paths) {
    const candidateLocationsPromises = [];
...
async function parsePluginInfo(pluginDir, packageName) {
    const pluginPath = path.join(pluginDir, packageName);
    const infoPath = path.join(pluginPath, 'package.json');
...
function isManagedUserPluginCopy(pluginPath) {
...
function cleanupStaleUserPluginCopy(stalePlugin, builtinPlugin) {
...
function resolveDuplicatePlugin(existing, candidate) {
    // 优先非 legacy 插件
...
export async function findPlugins() {
    ...
    const foundPlugins = [];
    const candidateLocations = await getPluginCandidateLocation(paths);
    const foundPluginsPromises = [];
...
export async function loadPlugins(foundPlugins, progress) {
    const plugins = [];
    ...
    for (const foundPlugin of foundPlugins) {
        pluginsPromises.push((async () => {
            try {
                ...
                const pluginManifest = normalizePluginManifest(manifestCandidate, foundPlugin.name);
